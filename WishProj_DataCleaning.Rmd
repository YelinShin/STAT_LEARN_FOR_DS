...
```{r}
wish <- wish %>% select(-title, -tags, -crawl_month,-theme,-product_id,-product_picture,-product_url,-merchant_id,-merchant_profile_picture,-merchant_info_subtitle,-merchant_name,-merchant_title,-urgency_text,-title_orig,-shipping_option_name, -currency_buyer)
summary(wish)
```
...
3. clean the sizes 
```{r}
wish %>% select(product_variation_size_id) %>% unique()
...
colnames(wish) = c('price','retail','sold_ct','ad_boost','rate','rate_ct','rate5','rate4','rate3','rate2','rate1','badge_ct','bg_local','bg_quality','bg_fastship','color','size','inventory','ship_price','is_express','ship_from','total_invent','has_bg_urgency','origin','seller_rate_ct','seller_rate','has_seller_propic')
