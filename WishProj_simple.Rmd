...
```{r}
library(rcompanion)
plotNormalHistogram(wish$sold_ct, main = 'sold_ct Histogram', xlab = 'sold_ct')
# since our sold_ct is right-skewed, we try performing log transfromation on it to make it normal distribution
plotNormalHistogram(log(wish$sold_ct), main = 'log(sold_ct) Histogram', xlab = 'log(sold_ct)') 
```
...
# svm with categorical sold_ct (data conversion in here)
...
tb <- table(x1)
par(mfrow = c(1, 2))
hist(log(wish$sold_ct), col = "grey", main = "log(sold_ct) as numerical", xlab = "log(sold_ct)")
barplot(tb, names.arg = row.names(tb), cex.names = 0.8, main = "sold_ct as categorical", las = 2)


wish.train <- wish_cate[train_rows, ]
...
summary(tuned) # cost = 1.51 is the best
```

```{r}
lin.svm <- svm(sold_ct_cate ~ ., kernel = "linear", type = "C-class", data = wish.train, cost = 1.51)

train_pred <- predict(lin.svm, wish.train)
table <- table(wish.train$sold_ct_cate, train_pred)

print("training error with cost = 1.51: ")
...
summary(tuned) # cost = 9.51 is best
...
```{r}
rad.svm <- svm(sold_ct_cate ~ ., kernel = "radial", data = wish.train, cost = 9.51)

train_pred <- predict(rad.svm, wish.train)
table <- table(wish.train$sold_ct_cate, train_pred)

print("radical svm - training error with cost = 9.51: ")
(table[2] + table[3]) / (sum(table[1:4]))
...
```{r}
poly.svm <- svm(sold_ct_cate ~ ., kernel = "poly", data = wish.train, degree = 3, cost = 9.51)

train_pred <- predict(poly.svm, wish.train)
table <- table(wish.train$sold_ct_cate, train_pred)

print("poly svm - training error with cost = 9.51: ")
(table[2] + table[3]) / (sum(table[1:4]))
