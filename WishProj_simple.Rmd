...
We check how many factors in each factor columns because we need to be careful when we split data. Error might occur if factor is in test set but not in training set.
...
set.seed(123)
# perform cross-validation to chose M
pls_model <- plsr(log(sold_ct) ∼ ., data = wish.train[, !(names(wish.train) %in% drops3)], scale = TRUE, validation = "CV")
summary(pls_model)
